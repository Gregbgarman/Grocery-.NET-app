...
namespace Project4_1.Items
{
    public class shoppingcart : INotifyPropertyChanged
    {
        ...
        public bool Contains(Guid id)                //method used to determine if an item is already in the cart by looking for the ID
        {
        ...
        public int GetIndex(Guid id)             //method runs if above method returns true and returns the index of the item already in cart
        {
            for (int i = 0; i < Cart.Count; i++)
            {
                if (Cart[i].ID == id)
